...
- Survives backgrounding mid-turn: plan-chat's tool-call loop + saving the turn to `chat_sessions` now run inside `EdgeRuntime.waitUntil`, so leaving the app while the agent is "thinking" no longer kills the turn or loses the action — the client detects a dropped connection, re-checks the session in the DB, and adopts whatever the server finished instead of just showing an error
- User messages are timestamped (`at`, ISO) at send time; plan-chat folds it into the model-facing copy as `[YYYY-MM-DD HH:MM]` so it can compute exact times/durations (e.g. splitting a logged block by when each part was mentioned) instead of asking
- OpenAI quota exhaustion (429 `insufficient_quota`, from either plan-chat or plan-transcribe) now surfaces as a plain "OpenAI credits are exhausted" chat bubble instead of a raw "openai error 429" (`_shared/openai-errors.ts`)
...
- **Buffered tool-call rounds**: a round that ends in a tool call is no longer streamed live token-by-token — it's buffered and only shown once the round is confirmed terminal, so a preamble that gets discarded server-side (the old `reset`-event case) can no longer flash on screen at all. Stop mid-reply on a buffered round now shows nothing rather than a partial (tradeoff accepted: buffering removes the typewriter effect for tool-calling turns, but a turn that streams plain text still types out live)
- **User Plan Preferences**: office schedule (per-weekday, date-ranged so "Mon–Thu office, Fri WFH from Aug" and "all-week office from Sept" coexist), 2026 holiday calendar, tiered wake times (ideal/acceptable/last-resort), and sleep rules (coffee/melatonin/nap/gym cutoffs, weekend wake flex, sunlight-after-wake) — set in `user_settings` + new `office_schedule_rules`/`holidays` tables, injected into every plan-chat turn as a PREFERENCES block (office/WFH/holiday mode for the day, wake-tier framing, sleep-rule cutoffs)
- **Push back, don't silently comply**: a request that conflicts with a stated preference (gym past cutoff, coffee past cutoff, a nap over the cap or past its cutoff, defaulting to the last-resort wake tier when nothing forces it) or the day's office/WFH/holiday mode gets named as a conflict with the specific rule cited, not silently scheduled — proceeds only if you confirm, same as a [PROTECTED] block flag
- **Explain the plan, not just propose it**: the prompt now requires citing the real evidence (sleep debt, gym gap, historical durations, the preference/mode just named) behind non-obvious block placement, so the reasoning in the reply makes a plan look intentional rather than arbitrary
- **Historical duration grounding**: block-time guesses for Lunch/Commute/Getting ready/Walk use your own median duration from the last 60 days (3+ occurrences) instead of a generic estimate, computed alongside the sleep/workout evidence snapshot
- **Fixed: compound corrections silently dropped the recount.** "I wasn't doing X, I was doing Y, and I'm starting Z now" is two edits (fix the mislabeled past period, then switch) — the model was pattern-matching only the trailing "starting Z now" clause to the simpler SWITCH rule and dropping the recount of Y entirely (confirmed via a real conversation: first attempt even claimed the recount had happened when no tool call for it had run, violating the existing "never claim an action you didn't take" rule). First fix (a new standalone COMPOUND CORRECTION bullet) did NOT change the behavior on retest — the competing SWITCH rule kept winning. Second fix patched the SWITCH rule itself with an explicit exception requiring update_time_entry on the running entry before the switch's start_timer call; verified live twice against the test account (first attempt after fix 1 still failed identically, second attempt after fix 2 produced the correct `Updated entry` → `Started` order, confirmed against the actual database).
- **Fixed: the model was blind to its own side effects.** `add_completed_entry`/`start_timer` can silently trim/split/move/delete OTHER entries to resolve an overlap (one-reality) — that was pushed only to `ctx.actions` (the UI's ✓ chips), never returned to the model itself, which only ever saw `{ ok: true, entry_id }` back from the tool call. The model was narrating the PRE-side-effect state from memory, producing self-contradictory replies (two activities described as overlapping when the database had already resolved them correctly) — root-caused by inspecting the exact tool-loop code (`content: JSON.stringify(result)`), not inferred from the transcript. Fixed generically in `runTool`: every call now returns a `db_changes` array (the ctx.actions pushed during that specific call), with a prompt rule to read and reflect it before describing the resulting timeline. Verified live: a mid-activity correction that trimmed a running "Deep work" entry now correctly reports "your Deep work entry was split around it" instead of staying silent on it — confirmed against the actual database, which matched the reply exactly. Codex review caught a real follow-up gap: `db_changes` was only attached to success results, so a side effect that ran before a LATER failure in the same call (e.g. add_completed_entry's overlap loop mutates other rows before its own insert, which can still error) got silently dropped exactly when it mattered most — fixed to attach `db_changes` regardless of success/error (verified by code inspection, not a forced-failure live repro — narrow edge case).
...
- **Instant rule-violation push**: starting a timer that breaks a stated sleep rule (coffee/nap/gym past its cutoff) pushes a notification immediately, not on the next brain tick — a `check_rule_violations` DB trigger on `time_entries` (matches gym by category or title) calls `brain` synchronously over the same pg_net→Vault pipeline the cron uses; skipped for backdated starts (>10 min in the past) so backfilling "I've been at the gym since 9:30" doesn't nag about something already underway
- **`late-wake` concern**: fires when 3+ of the last 5 office-mode wake times hit your last-resort tier — an undated, self-clearing concern (like `gym-gap`) that re-evaluates every run instead of firing once per date
- **Fixed: sleep-debt's recommended bedtime anchored to your own drifted-late wake time, not your goal.** `recBedClock` was `medianRecentWakeTime − targetSleepHours`, so on the real account it recommended 2:38am — later than even the last-resort wake tier — because it was reverse-engineering a bedtime from a week of already-bad wake times instead of working toward the stated `wake_ideal_time` (07:30). `computeSleepEvidence` (shared by `plan-chat` and `brain`) now anchors `recBedClock` to `wake_ideal_time` when set, falling back to the old median-wake behavior otherwise; verified against real data (07:30 ideal wake, 7h floored target → 12:30am, matching by hand-calculation). This makes the number a stable target (like a calorie goal) rather than one that chases wherever sleep drifted — the debt figure itself still moves, the goalpost doesn't
...
- **Fixed: gym sessions matched on exact title "Gym"** (missed real sessions with a descriptive title, e.g. "Gym – upper body (push + pull)"), then a substring fix shipped and was found live-wrong (it also matched "Getting ready for gym" and "Commute to gym", inflating the count — confirmed on the real account, 3 counted vs. 1 actual). Root cause: `start_timer`/`add_completed_entry` had no `tags` parameter at all, so the agent had no way to tag a session even when told to — every agent-created entry got `tags: []`, breaking the "gym" tag that manually-logged entries had carried consistently (9 of the last 10 real sessions had it). Fixed at the source: both tools now accept `tags`, the prompt tells the model to always tag a gym session `["gym", <type>]`, and all four gym-matching sites (`getGymFrequencyTrend`, `getSleepGymCorrelation`, `computeWorkoutEvidence`, and `brain`'s instant rule-violation trigger) match on title-prefix OR the "gym" tag — verified live end-to-end (agent-started "Leg day workout" correctly tagged `["gym","legs"]`)
...
- **"Did it work" made legible**: was unlabeled and showed up to 5 rows verbatim, often 4 of them the exact same title+resolution repeated with nothing to tell them apart ("No plan for today yet / condition cleared" ×4). Added a one-line explainer under the header, a relative-time stamp per row, and consecutive identical rows now collapse into one with a ×N count instead of repeating
...
- Broad insight cards, one per category, across everything tracked (not just Sleep/Gym): new `pulse-insights` edge function computes trend (intensity, current 7d vs baseline 21d, per-active-day median), attendance (frequency — has this category gone quiet relative to its own normal rhythm?), and fragmentation per category server-side, gated only on whether there's enough history to trust a comparison — never on how big the change is, since a fixed magnitude cutoff misses real signal in stable categories and floods noisy ones. One gpt-5.1 call phrases every real signal into a card; no card cap (Pulse already scrolls). Loads independently of the other cards so a slow LLM call can't blank the rest of the page. Validated over 4 rounds against real + synthetic data before being written (`scratchpad/insight-eval/`)
...
- **Cross-midnight entries now count toward today**: `getEntriesForDate`'s local-day filter matched on `start_time` only, so an entry that started before local midnight and ran past it (a late-night session ending 3:30am) was excluded from "today" entirely — the tracked-hours total undercounted and the 24h bar showed a false-empty stretch at the start of the day, even though that time was genuinely covered. Filtering now checks whether the entry's whole span *overlaps* the local day, not just where it starts (`overlapsLocalDay` in `day-range.ts`); the downstream clamp-to-day-bounds logic (already correct) then renders only the today-side portion.
...
- Agent-generated titles (start_timer, add_completed_entry, add_calendar_block, propose_plan items) now stay as plain as the user's own words instead of the model inventing a fuller phrasing ("work" → "Office work - focused block"). First pass only covered todo-linked propose_plan items; broadened after finding the actual reported case had no linked todo at all (an ad-hoc add_calendar_block item) — this class of tool call had no title guidance whatsoever. Plan-vs-actual matches by category+time only (title was never load-bearing there), but Insights' Plan Drift table and Home's "next" line both display the plan's title next to the tracked outcome, so a reworded title made a fully-honoured plan visually read as unmet.
...
- **Onboarding example now fades out with use**: the empty-chat "Plan your day, out loud or by text" title + example paragraph only shows for a first-time user or one who hasn't chatted in 7+ days (checked via the most recent `chat_sessions.updated_at` across all dates); the quick-prompt chips ("Plan my day", "Replan from now"…) stay regardless — an active user was seeing the full explanation on every fresh chat, which read as clutter once the pattern is known
- **Voice errors don't lose the recording**: a failed transcription (no audio, network drop, upload failure) no longer pops a blocking alert — it shows an inline `⚠️` bubble with a **Retry** chip that re-transcribes the same clip (no re-recording needed)
...
- **Task vs. scheduled event**: a commitment with an explicit day+time ("Monday 6pm interview") always becomes a calendar block, never a backlog todo. Something that sounds like it belongs on the calendar (an interview, meeting, appointment) but arrives with no time asks for it instead of silently parking it as a vague task; genuinely open-ended asks ("call the plumber sometime") still go straight to the backlog
...
- **Real word-by-word streaming**: the reply text now streams token-by-token as the model writes it (like ChatGPT), not a spinner-then-pop-in blob. The plan proposal moved out of the reply's structured JSON into a `propose_plan` tool call (alongside the other 20 agent tools) — that's what unlocked plain streamable text; a `clear_plan` tool lets the model explicitly scrap a plan the user rejects
...
- **No more stray text before a tool call**: a round that ends in a tool call is buffered server-side and only sent once terminal, so a preamble the model streams before deciding to call a tool never reaches the client at all (superseded the earlier `reset`-SSE-event approach, which cleared the fragment after briefly displaying it — see "Buffered tool-call rounds" above)
- **Web search**: the agent can look up genuinely time-sensitive things (weather, prices, live results) it can't know from training — a `web_search` tool hands the query off to a separate, isolated `gpt-4o-search-preview` call (gpt-5.1 itself can't swap to a search model mid-conversation without losing its other 20 tools) and relays the grounded answer back. No new API key — same OpenAI key already used for chat
...
- Fixed: the live counter (Lock Screen/Dynamic Island AND the in-app timer) reset to 0 whenever an agent correction had to replace the running entry with a new row. Root cause: `update_time_entry` can only ever close an entry (set end_time), never reopen one — so undoing a mislabeled running entry (e.g. the agent starts "Nap" from a message that only mentioned "nap" in passing) always means delete + `start_timer` a fresh row, and both the in-app elapsed clock (`useTimer.ts`) and the Live Activity (`liveActivity.ts`) were keyed straight to `entry.start_time`, so the new row's start became the new zero point even though the real activity was continuous. Added `getSessionStart` (`time-entries.ts`): walks backward through same-title/category predecessor entries that end within 90s of the current one's start (sized to the delete+start_timer round-trip, not general break-bridging) to recover the true session start; both `useTimer`'s elapsed and the Live Activity's `elapsedTimer.startDate` now use it. Verified against the real failing case: a running entry split by exactly this correction chain (30s gap) correctly resolves back to its true start 28 minutes earlier, and correctly stops at a real gap (the mislabeled Nap period) one hop further back — needs a new IPA build to reach the device.
...
- **Fixed: a wide backfill silently ate already-logged entries inside its window.** `add_completed_entry`'s overlap-resolution soft-deletes anything it covers, and the agent had no way to see soft-deletes — so it also told the user recovered data was gone for good, when the row was untouched in the DB. `list_time_entries` now also returns `recently_removed` (soft-deletes from the last 7 days overlapping that date, including still-open running entries — an earlier version of this fix wrongly excluded those) and a new `restore_time_entry` tool brings one back — trimming/splitting whatever now occupies its old window (shared `resolveOneRealityOverlap`, extracted from `add_completed_entry`), and closing it at the moment it was removed rather than resuming it as running (something else may be running now). The system prompt now tells the agent to only backfill actual gaps when existing entries sit inside the recounted window, not blanket the whole span. All three sub-fixes verified live against the deployed function (test account, sandbox dates) after an independent review caught them.
