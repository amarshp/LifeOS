...
- **Fixed: a wide backfill silently ate already-logged entries inside its window.** `add_completed_entry`'s overlap-resolution soft-deletes anything it covers, and the agent had no way to see soft-deletes — so it also told the user recovered data was gone for good, when the row was untouched in the DB. `list_time_entries` now also returns `recently_removed` (soft-deletes from the last 7 days overlapping that date) and a new `restore_time_entry` tool brings one back exactly as it was; the system prompt now tells the agent to only backfill actual gaps when existing entries sit inside the recounted window, not blanket the whole span.
