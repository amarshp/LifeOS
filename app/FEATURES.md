...
> - **Dedicated Stop shortcut (IMPLEMENTED, pending device test)**: `StopIntent` ("Stop \<task\> in/on LifeOS", "End \<task\> in LifeOS") + `StopDictateIntent` ("Stop in LifeOS" → dictate) prepend "stop " so the same parser stops the entry and the intent ends the card. Natural one-step stop without the "track stop …" workaround.
