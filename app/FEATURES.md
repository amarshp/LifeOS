...
- **Fixed: gym sessions matched on exact title "Gym"**, which missed real sessions logged with a descriptive title (e.g. "Gym – upper body (push + pull)") — there's no dedicated Gym category on the real account (workouts land under generic "Activity"), so title is the only reliable signal. All three gym-matching queries (`getGymFrequencyTrend`, `getSleepGymCorrelation`, and `computeWorkoutEvidence` — which also feeds `brain`'s gym-gap concern and plan-chat's evidence snapshot) switched from exact match to a case-insensitive substring match, same convention already used for Sleep
