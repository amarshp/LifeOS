...
- **Layout**: top row is the Plan/Tasks toggle (chip + underline style) with the TTS speaker icon on the right — no separate title row. Date selector (just the date, tap ‹ › to step) sits below the chat, right above the input bar; on the Tasks tab it stays at the top since "+ plan" needs the target date up front
