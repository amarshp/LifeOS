...
> - **Dedicated Stop shortcut**: `StopIntent` + `StopDictateIntent` prepend "stop " so the same parser stops the entry and the intent ends the card. Phrasing settled on **"end"** (Siri reserves the word "stop" as a system command, so leading-"stop" phrases like "LifeOS stop X" get hijacked and fail). Working set: one-step "End \<task\> in LifeOS" / "Stop \<task\> in LifeOS" / "LifeOS end \<task\>"; two-step "**LifeOS end**" → dictate, "End in LifeOS", "Stop in LifeOS". Start mirror: "Track \<task\> in LifeOS", "**LifeOS track**" → dictate.
