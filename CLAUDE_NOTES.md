...
### Navigation restructure: Timeline + Agent + Tasks (2026-07-03 night → 2026-07-04)
- User confirmed "Agent" rename, then asked for Tasks as its own bottom tab, and Day+Week merged into a Timeline toggle — "do this QC it and close it up, I want to start another task." Full scope, one shot, no back-and-forth expected.
- Mapped the ENTIRE blast radius with an Explore agent before touching anything: 7 call sites push to `/day` with a `sheet|date|editEntry|focusTs` param contract (Live Activity stop button, Siri, push notifications, search, review, home, week's own day-tap, plan's "View day") — none of these could be allowed to break. Also found `showWeekTab` had 10 read/write sites across SettingsContext + _layout + settings.tsx, and confirmed TodoBacklog had exactly one consumer (safe to move wholesale).
- **Key risk-reduction decision**: kept the `/day` ROUTE unchanged (renamed only the tab's display title to "Timeline") specifically so none of those 7 call sites needed touching — a route rename would have cascaded into native-adjacent code (Live Activity deep link redirect in stop-start.tsx) for zero user-visible benefit.
- Extracted week.tsx's ENTIRE rendering (gestures, zoom, lanes, grid math) byte-for-byte into `src/components/WeekTimeline.tsx`, changing only the day-tap handler from `router.push` to an `onSelectDay` callback prop. day.tsx's Day-mode internals (the pinch/scroll timeline, ~1300 lines) were untouched — only re-wrapped: outer `styles.safe` View is now unconditional (holds the new Day/Week toggle + FABs/sheets), and the OLD outer swipeGesture+timeline block became a `viewMode==='day'` conditional branch. Added `jumpToDate()` (absolute jump, mirrors the existing `params.date` effect) for the week→day tap-through.
- Tasks lifted out of plan.tsx into `app/(tabs)/tasks.tsx` — its own `date` state now (decoupled from Agent's planning-session date, more correct anyway). Agent tab reverted to a plain "Agent" title header, no more segmented control.
- Removed `showWeekTab` end-to-end (not repurposed) — with Week always reachable via the in-Timeline toggle, a "hide the Week tab" setting has no meaning left.
