-- Codex review caught this: voice_track_admin's "stop" path returned a
-- success-shaped response (action='stop', is_running=false) even when no
-- running entry matched — entry_id/title/category_id were just null, so a
-- caller couldn't tell "stopped" from "nothing to stop". Only the stop
-- branch changes; rest of _voice_track_core is identical to 20260817_001.
...
BEGIN
    ...
    IF v_action = 'stop' THEN
        ...
        -- No running entry matched at all -> say so explicitly instead of
        -- returning a stop-shaped response with everything null.
        IF v_match.id IS NULL THEN
            RETURN jsonb_build_object('action', 'stop', 'error', 'no running entry to stop', 'entry_id', NULL);
        END IF;
