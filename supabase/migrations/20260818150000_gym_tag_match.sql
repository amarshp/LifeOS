-- Match gym on title prefix OR a "gym" tag, same signal now used client-side
-- (lifeSignals.ts) and server-side (wellness-evidence.ts computeWorkoutEvidence)
-- — the tag is the more durable signal (manually-logged entries carry it
-- consistently; an agent-created entry with a descriptive title and no tag
-- is exactly what caused the gym-detection bug this trigger already had a
-- narrower version of). Title prefix stays as a fallback.
...
BEGIN
    ...
    ELSIF v_gym_cutoff IS NOT NULL AND v_local_time >= v_gym_cutoff
        AND (v_title_lower LIKE 'gym%' OR NEW.tags @> ARRAY['gym']) THEN
