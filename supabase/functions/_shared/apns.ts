...
export interface UpdatePush {
  token: string // per-ACTIVITY update token (not the push-to-start token)
  title: string
  subtitle: string
  startMs: number // keep the elapsed timer anchored — omitting it resets the count
}

// Update a live activity's content state (e.g. refresh the "Next: …" subtitle)
// while the app is closed. Sent to the activity's own token.
export async function sendActivityUpdate(p: UpdatePush): Promise<number> {
  const host = Deno.env.get('APNS_HOST') ?? 'https://api.push.apple.com'
  const topic = Deno.env.get('APNS_TOPIC')
  if (!topic) throw new Error('APNS_TOPIC not set')
  const jwt = await makeJwt()
